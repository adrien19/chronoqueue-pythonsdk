"""
Async ChronoQueue Client with async/await support.

This module provides an asynchronous version of the ChronoQueue client that uses
...
Example:
    ...
    >>>
    >>> async def main():
    ...     client = AsyncChronoqueueClient(host="localhost", port=50051, use_tls=False)
...
class AsyncChronoqueueClient:
    """
    Asynchronous client for Chronoqueue with async/await support.
    ...
    def __init__(
        ...
        """
        Initialize the AsyncChronoqueueClient.
    ...
    async def connect(self):
        """
        Establish connection to the Chronoqueue service.
        ...
        logging.info(f"Connected to Chronoqueue at {self.host}:{self.port}")
    ...
    async def create_queue(self, name: str, options: Optional[QueueOptions] = None, error_handler=None) -> ResponseWrapper:
        ...
        -------
        ResponseWrapper
            A wrapper around the gRPC response from the Chronoqueue service.
        ...
        Example:
        --------
        >>> from chronoqueue.utils import QueueOptions
    ...
    async def create_schedule(self, schedule_id: str, options: ScheduleOptions, error_handler=None) -> ResponseWrapper:
        ...
        Example:
        --------
        >>> from chronoqueue.utils import ScheduleOptions
        >>> options = ScheduleOptions(...)
    ...
    async def list_schedules(self, prefix: str = "", error_handler=None) -> ResponseWrapper:
        """
    ...
    async def get_schedule_history(self, schedule_id: str, limit: int = 10, error_handler=None) -> ResponseWrapper:
        """
        ...
        Example:
        --------
        >>> response = await client.get_schedule_history("my_schedule", limit=20)
        """
        try:
    ...
    async def list_schemas(
        ...
        Example:
        --------
        >>> response = await client.list_schemas(prefix="order_", limit=50)
        """
        try:
    ...
    async def get_dlq_messages(self, dlq_name: str, limit: int = 100, error_handler=None) -> ResponseWrapper:
        """
        ...
        Example:
        --------
        >>> response = await client.get_dlq_messages("orders_queue_dlq", limit=50)
    ...
    async def close(self, timeout: float = 30.0):
        ...
        >>> await client.close()
        """
        logging.info("Closing AsyncChronoqueueClient, stopping all heartbeats...")
        ...
        if self.channel:
            ...
            logging.info("AsyncChronoqueueClient closed successfully")
